...
int main(int argc, char *argv[]) {
  ...
  rem = ARRAY_SIZE - num_blocks * THREADS_PER_BLOCK;
  if (rem > 0)
  {
    num_blocks++;
  }
  ...
  printf("Running with %i blocks x %i threads/block for %i elements\n",
  ...
  err = cudaMemcpy(h_out, d_a, sz, cudaMemcpyDeviceToHost);
